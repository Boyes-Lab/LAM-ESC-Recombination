...
print('Welcome to the LAM-ESC analysis script')
print('')
print('Required: flash https://ccb.jhu.edu/software/FLASH/ ')
...
print('')
print('Example: python3 lam-esc.py sample_name folder_path')
...
try:
	...
	print('Error: sample name and or analysis folder path not provided')
...
print('Running LAM-ESC analysis! This might take a while....')
print('')
...
barcodes['IGKJ1'] = 'CCACACCTGCTCGAAAAGGG'
barcodes['IGKJ2'] = 'GTCCTGGGGAACATAACATGTTCTGG'
barcodes['IGKJ3'] = 'TGTTCCACAGGACGCTAGCTTGTG'
barcodes['IGKJ4'] = 'GGGGAAATGGGGAAGAAATAGCTAG'
barcodes['IGKJ5'] = 'GCAAGATTTTCTCTGCATCGGTCAGG'
barcodes['KDE'] = 'GAGCAGACTGTGAGGGACACG'
barcodes['IGLJ1'] = 'CAACCCTGTGCCCGTCATGC'
barcodes['IGLJ23'] = 'CTCATGAGCAGATGCCACCAGG'
...
def run_analysis(gene):
    ...
    command = f'{blastn} -query {gene}_tmp.fasta -db {blast_db_path}/blast_db/{gene}_gene_segment.fasta -outfmt {outfmt} -evalue {evalue} -max_target_seqs {max_target_seqs} -task blastn -num_threads {cpu_threads} > {gene}_tmp_blast_output.txt'
    os.system(command)
    ...
    if 'IGK' in gene:
        blast_db = f'{blast_db_path}/blast_db/IGKV_RSS_ext.fasta'
    if 'KDE' in gene:
        blast_db = f'{blast_db_path}/blast_db/IGKV_RSS_ext.fasta'
    if 'IGL' in gene:
        blast_db = f'{blast_db_path}/blast_db/IGLV_RSS_extended.fasta'
    ...
    if 'IGK' in gene:
        blast_db = f'{blast_db_path}/blast_db/{gene}_SJ.fasta'
    if 'KDE' in gene:
        blast_db = f'{blast_db_path}/blast_db/{gene}_SJ.fasta'
    if 'IGLJ1' in gene:
        blast_db = f'{blast_db_path}/blast_db/{gene}_SJ.fasta'
    if 'IGLJ23' in gene:
        blast_db = f'{blast_db_path}/blast_db/IGLJ2_SJ.fasta'
    ...
    with open(f'{sample_name}_{gene}_ESC_blast_result_top_two_tmp.tsv','w') as f:
        for item in output:
            f.write(f'{item}')
            
    command = "sort -t '\t' -k1,1 -k11g,11 " + f'{sample_name}_{gene}_ESC_blast_result_top_two_tmp.tsv > {sample_name}_{gene}_ESC_blast_result_top_two.tsv'
    os.system(command)
    os.remove(f'{sample_name}_{gene}_ESC_blast_result_top_two_tmp.tsv')
...
for gene in genes:
    ...
    #to exclude where != 2
    with open(f'{sample_name}_{gene}_ESC_blast_result_top_two.tsv','r') as f:
        ids = [line.split('\t')[0] for line in f]
    output = []
    with open(f'{sample_name}_{gene}_ESC_blast_result_top_two.tsv','r') as a:
        for line in a:
            ident = line.split('\t')[0]
            if ids.count(ident) !=2:
                ...
                output.append(line)
                a.readline()
        with open(f'{sample_name}_{gene}_ESC_blast_result_top_hit.tsv','w') as b:
            for item in output:
                b.write(item)
...
for gene in genes:
    with open(f'{sample_name}_{gene}_ESC_blast_result_top_hit.tsv','r') as f:
        for line in f:
            try:
...
with open(f'{sample_name}_ESC_summary.txt','w') as f:
...
for gene in genes:
    ...
    with open(f'{sample_name}_{gene}_ESC_blast_result_top_two.tsv','r') as a:
        ...
        with open(f'{sample_name}_{gene}_ESC_blast_result_top_hit_e_val.tsv','w') as b:
            for item in output:
                b.write(item)
...
recombination_events = []
for gene in genes:
    with open(f'{sample_name}_{gene}_ESC_blast_result_top_hit_e_val.tsv','r') as f:
        for line in f:
            try:
                recombination_event = line.split('\t')[1]
                # E value filter 1e-21
...
with open(f'{sample_name}_ESC_summary_unique_e_val.txt','w') as f:
    for item in x.most_common():
        f.write(f'{item}\n')
